...
const DB = {
  // Default programme structure (from Matt's spreadsheet)
  defaultProgramme: [
    {
  ...
  // ── STORAGE HELPERS ──
  save(key, val) { try { localStorage.setItem('mpl_' + key, JSON.stringify(val)); } catch(e) {} },
  load(key, fallback = null) {
    try { const v = localStorage.getItem('mpl_' + key); return v !== null ? JSON.parse(v) : fallback; }
  ...
  clearActive() { try { localStorage.removeItem('mpl_active'); } catch(e) {} },
  ...
  // ── PROGRESSION: compare this set vs last session same exercise ──
  getLastSession(dayId, exerciseId) {
    const sessions = this.getSessions();
    const past = sessions.filter(s => s.dayId === dayId && s.id !== (this.getActive() || {}).id);
    if (past.length === 0) return null;
    const last = past[0];
    const ex = (last.exercises || []).find(e => e.id === exerciseId);
    return ex || null;
  },

  // ── get e1RM history for an exercise across sessions ──
  getE1RMHistory(exerciseId) {
    ...
    for (const s of sessions) {
      ...
      if (maxE1RM > 0) hist.push({ date: s.date, week: s.week, e1rm: maxE1RM, dayLabel: s.dayLabel });
    }
    return hist;
  },
  ...
  getProgressionStatus(dayId, exerciseId, currentSets) {
    ...
    if (lastSets.length === 0) return 'new';
    ...
    if (curSets.length === 0) return null;
  ...
  getDeloadAnalysis() {
    ...
    for (const ex of allExercises) {
      ...
      if (isStalled) {
        stalledCount++;
        stalled.push({ name: ex.name, weeks: recent.length, e1rm: recent[recent.length-1].e1rm });
      }
    }
    const sessions = this.getSessions();
    const recentSessions = sessions.slice(0, 12);
    const avgRIR = recentSessions.flatMap(s => s.exercises || [])
      .flatMap(e => e.sets || []).filter(s => s.rir !== '' && s.rir !== undefined)
      .map(s => parseFloat(s.rir)).filter(v => !isNaN(v));
    ...
    if (fatigueScore >= 70 || stallPct >= 0.6) {
      ...
      status = 'monitor';
      recommendation = 'Some stalls detected. Monitor closely. Consider technique reset on flagged exercises before triggering full deload.';
    } else {
      recommendation = 'Progress is on track. Continue current programme. Reassess after next 2 sessions.';
    }
