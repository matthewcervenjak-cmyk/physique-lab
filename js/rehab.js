...
const Rehab = {
  ...
  renderRehab() {
    ...
    let html = `
      <div class="section-header">
        <div>
          <div class="section-title">Rehab</div>
          <div class="section-sub">Custom recovery tracking</div>
        </div>
    ...
    if (exercises.length === 0) {
      ...
      for (const ex of exercises) {
        const lastSession = DB.getRehabSessions().find(s => s.exerciseId === ex.id);
          ...
          <div class="card rehab-ex-card">
            ...
            <div style="padding:0 14px 4px">
              ...
              <div class="set-row rehab-set-row" style="grid-template-columns: 26px 1fr 1fr 36px" id="rehab-set-${ex.id}-${i}">
                ...
                <input class="set-input" type="number" inputmode="decimal" placeholder="kg" value="${set.load||''}"
            ...
            <div class="prev-row">
              <span class="prev-item rehab-note">${lastText}</span>
            </div>
            <div style="padding:10px 14px 14px">
              <button class="btn-primary" style="margin-top:0;font-size:14px;padding:12px" onclick="Rehab.logSession('${ex.id}')">
                Save Session
              </button>
            </div>
          </div>`;
      ...
      const rehabSessions = DB.getRehabSessions().slice(0,10);
      if (rehabSessions.length > 0) {
        html += `
          <div class="section-header" style="margin-top:8px">
            <div class="section-title" style="font-size:16px">Recent sessions</div>
          </div>`;
            ...
            <div class="card">
              <div class="pad-card">
                <div class="flex-between" style="margin-bottom:6px">
                  <span style="font-size:13px;font-weight:600;color:var(--accent,#fc7ab0)">${ex ? ex.name : 'Unknown'}</span>
                  <span class="text-muted text-sm">${DB.formatDate(s.date)}</span>
                </div>
                ${s.sets.filter(st=>st.reps).map((st,i)=>`
                  <div class="flex-between" style="font-size:12px;padding:3px 0;border-bottom:1px solid var(--border,#2a1220)">
                    <span class="text-muted">Set ${i+1}</span>
                    <span style="color:var(--text2,#d8d0d4)">${st.reps} reps${st.load?' @ '+st.load+'kg':''}</span>
                  </div>`).join('')}
                ${s.note ? `<div class="text-muted text-sm mt-4">${s.note}</div>` : ''}
              </div>
            </div>`;
        }
      }
  ...
  // Track in-memory log sets per exercise
  _logSets: {},
  ...
  updateSet(exId, idx, field, value) {
    ...
    DB.saveRehabExercises(exercises);
  },
  ...
  addExercise() {
    App.showModal('Add rehab exercise', `
      <div class="form-group">
        ...
        <input class="form-input" id="rehab-new-name" placeholder="e.g. Hip flexor stretch">
      </div>
      <div class="form-group">
        <label class="form-label">Notes / instructions (optional)</label>
        <input class="form-input" id="rehab-new-notes" placeholder="e.g. Hold 30s each side">
      </div>
  ...
  confirmAddExercise() {
    ...
    exercises.push({
      id: DB.newId(),
      name,
      notes: document.getElementById('rehab-new-notes').value.trim(),
      logSets: [{ reps:'', load:'' }, { reps:'', load:'' }, { reps:'', load:'' }],
    });
    DB.saveRehabExercises(exercises);
    App.closeModal();
    this.renderRehab();
  ...
  editExercise(exId) {
    ...
    App.showModal(`Edit: ${ex.name}`, `
      <div class="form-group">
        <label class="form-label">Exercise name</label>
        <input class="form-input" id="rehab-edit-name" value="${ex.name}">
      </div>
      <div class="form-group">
        <label class="form-label">Notes / instructions</label>
        <input class="form-input" id="rehab-edit-notes" value="${ex.notes||''}">
      </div>
      <button class="btn-primary" onclick="Rehab.saveExercise('${exId}')">Save</button>
      <button class="btn-secondary" style="color:var(--red,#fc6a4a);border-color:#5a1a1a" onclick="Rehab.deleteExercise('${exId}')">Delete</button>
      <button class="btn-secondary" onclick="App.closeModal()">Cancel</button>`);
  },
